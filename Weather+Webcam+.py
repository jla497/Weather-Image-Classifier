...
#check if img subfolder exists. If not throw error.
if os.path.isdir(imgpath)==False:
    print("Missing img folder")
    

if os.path.isdir(os.getcwd()+'/yvr-weather') == False:
    print("Missing yvr-weather folder")
    

#Check if image dataset has been processed. If not, process data first.
if os.path.isdir(imgpath+'/training') == False or os.path.isdir(imgpath+'/validation')==False:
    process_data()
...
def main():
    ...
    #Referenced model from Keras documentation
    model = Sequential()
